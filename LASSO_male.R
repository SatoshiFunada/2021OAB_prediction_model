...
data <- read_csv("data/data_male.csv")
data_cc <- data %>% 
  drop_na()

##################
#set up original data
LR1<-lrm(OAB_2~age+ bmi+ smoke+ alcohol+ walking+
           HT+ HL+ DM+ MI+ STROKE+   Renal_disease+ CANCER+ Depression+ Insomnia+ SAS+  Prostate_disease+ PCa+
           OABSS1_1+ OABSS2_1+ OABSS3_1+ OABSS4_1,
         data = data_cc,
         x=T, y=T,linear.predictors = T)

LR2<-lrm(OAB_2~age+ bmi+ smoke+ alcohol+ walking+
           HT+ HL+ DM+ MI+ STROKE+   Renal_disease+ CANCER+ Depression+ Insomnia+ SAS+  Prostate_disease+ PCa+
           OABSS1_1+ OABSS2_1+ OABSS3_1+ OABSS4_1+
           HbA1c+ eGFR+ BNP+ PSA,
         data = data_cc,
...
probLASSO1 <- predict(LR.L1, newx =LR1$x , type = "response")
probLASSO2 <- predict(LR.L2, newx =LR2$x , type = "response")
...
#Calibration
val.prob(p.LASSO1$s0, data_cc$OAB_2, m=200, cex=1)[c("Intercept","Slope")]
val.prob(p.LASSO2$s0, data_cc$OAB_2, m=200, cex=1)[c("Intercept","Slope")]
...
#LR.L2 <- update(LR.L1, ~ . +HbA1c+eGFR+BNP+CRP)
#DCA_data$predictorsmodel_LASSO2 = predict(LR.L2, type="response")
